...
export const getPostsByUserId = (userId) => (dispatch) => {
  dispatch(loadPosts());
  axios
    .get(`http://localhost:7000/api/posts/${userId}`)
    .then((res) =>
      dispatch({
        type: GET_POSTS,
        payload: res.data,
...
export const followUser = (userId) => (dispatch) => {
  axios
    .post("http://localhost:7000/api/users/follow", { userId })
    .then((res) =>
      dispatch({
        type: FOLLOW,
...
export const unfollowUser = (userId) => (dispatch) => {
  axios
    .post("http://localhost:7000/api/users/unfollow", { userId })
    .then((res) =>
      dispatch({
        type: UNFOLLOW,
        payload: res.data.userId,
...
export const searchUser = (searchData, history) => (dispatch) => {
  axios
    .post("http://localhost:7000/api/users/search", searchData)
    .then((res) => {
